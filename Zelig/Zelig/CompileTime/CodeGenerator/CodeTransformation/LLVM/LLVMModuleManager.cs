    ...
    public partial class LLVMModuleManager
    {
        ...
        private void AddToGlobalInitializedValues( IR.DataManager.DataDescriptor dd, _Value global )
        {
            ...
            else 
            {
                ...
                SetMutabilityFlagsAndSection( m_globalInitializedValues[ dd ], dd );
            }
        }

        private void SetMutabilityFlagsAndSection( _Value value, IR.DataManager.DataDescriptor dd )
        {
            var gv = value.LlvmValue as GlobalVariable;
            Debug.Assert( gv != null );

            gv.IsConstant = !dd.IsMutable;
            if( !dd.IsMutable )
            {
                // for real constant data mark it as unnammed_addr to allow the backend to coalesce
                // identical content. (e.g. if three globals are declared of the same type with the
                // same constant data initializer, then only one copy is really required in the final
                // image generated by the linker.
                gv.UnnamedAddress = true;
            }

            gv.Section = m_SectionNameProvider.GetSectionNameFor( dd );
        }
