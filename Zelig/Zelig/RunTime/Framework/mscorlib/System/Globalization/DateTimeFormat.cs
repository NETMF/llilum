...
////    private const int DEFAULT_ALL_DATETIMES_SIZE = 132;
        ...
        private static void FormatDigits( StringBuilder outputBuffer, int value, int len )
        {
            ...
            do
            {
                buffer[--offset] = (char)(n % 10 + '0');
                n /= 10;
            } while((n != 0) && (offset > 0));
    
            int digits = (int)(16 - offset);
            ...
            while((digits < len) && (offset > 0))
            {
                buffer[--offset] = '0';
                digits++;
            }
            outputBuffer.Append( buffer, offset, digits );
        }
                    ...
                    case 'z':
                        ...
                        else
                        {
                            if(dateTime.Kind == DateTimeKind.Utc)
                            {
                                ...
                                // explicitly emit the local time offset.
...
////                dateTime = dateTime.ToUniversalTime();
        ...
        internal static String Format( DateTime dateTime, DateTimeFormatInfo dtfi )
        {
            string format = dtfi.GeneralLongTimePattern;
                ...
                switch(ch)
                {
                    ...
                    case 'y':
                        ...
                        ////                    else
                        {
                            ...
                            {
                                FormatDigits( result, year, tokenLen );
                                //String fmtPattern = "D" + tokenLen;
                                //result.Append( year.ToString( fmtPattern, CultureInfo.InvariantCulture ) );
                            }
                        }
