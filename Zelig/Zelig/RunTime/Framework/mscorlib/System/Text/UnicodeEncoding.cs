...
////[System.Runtime.InteropServices.ComVisible( true )]
    public class UnicodeEncoding : Encoding
    {
        ...
        public UnicodeEncoding( bool bigEndian, bool byteOrderMark, bool throwOnInvalidBytes ) : base( bigEndian ? 1201 : 1200 )  //Set the data item.
        {
...
////    [System.Runtime.InteropServices.ComVisible( false )]
        ...
        public override unsafe int GetBytes( String s         ,
                                             int    charIndex ,
                                             int    charCount ,
                                             byte[] bytes     ,
                                             int    byteIndex )
...
////    [System.Runtime.InteropServices.ComVisible( false )]
        ...
        internal override unsafe int GetByteCount( char*      chars   ,
                                                   int        count   ,
                                                   EncoderNLS encoder )
        {
            ...
            while(((ch = (fallbackBuffer == null) ? (char)0 : fallbackBuffer.InternalGetNextChar()) != 0) || chars < charEnd)
            {
                ...
                if(ch >= 0xd800 && ch <= 0xdfff)
                {
                    ...
                    if(ch <= 0xdbff)
                    {
                        ...
                        if(charLeftOver > 0)
                        {
                            ...
                            // advanced already.
                            BCLDebug.Assert( chars > charStart, "[UnicodeEncoding.GetByteCount]Expected chars to have advanced in unexpected high surrogate" );
                            chars--;
                ...
                else if(charLeftOver > 0)
                {
                    ...
                    BCLDebug.Assert( chars > charStart, "[UnicodeEncoding.GetByteCount]Expected chars to have advanced when expected low surrogate" );
                    chars--;
            ...
            BCLDebug.Assert( fallbackBuffer == null || fallbackBuffer.Remaining == 0, "[UnicodeEncoding.GetByteCount]Expected empty fallback buffer at end" );

            // Don't remember fallbackBuffer.encoder for counting
            return byteCount;
        }

        internal override unsafe int GetBytes( char*      chars     ,
                                               int        charCount ,
                                               byte*      bytes     ,
                                               int        byteCount ,
                                               EncoderNLS encoder   )
        {
            ...
            while(((ch = (fallbackBuffer == null) ? (char)0 : fallbackBuffer.InternalGetNextChar()) != 0) || chars < charEnd)
            {
                ...
                if(ch >= 0xd800 && ch <= 0xdfff)
                {
                    ...
                    if(ch <= 0xdbff)
                    {
                        ...
                        if(charLeftOver > 0)
                        {
                            ...
                            // don't have leftover data in the fallback, so chars must have
                            // advanced already.
                            BCLDebug.Assert( chars > charStart, "[UnicodeEncoding.GetBytes]Expected chars to have advanced in unexpected high surrogate" );
                            chars--;
                    ...
                    if(bytes + 3 >= byteEnd)
                    {
                        ...
                        else
                        {
                            ...
                            BCLDebug.Assert( chars > charStart + 1 || bytes == byteStart,"[UnicodeEncoding.GetBytes]Expected chars to have when no room to add surrogate pair" );
                ...
                else if(charLeftOver > 0)
                {
                    ...
                    // this should be safe because we don't have leftover data in the
                    // fallback, so chars must have advanced already.
                    BCLDebug.Assert( chars > charStart, "[UnicodeEncoding.GetBytes]Expected chars to have advanced after expecting low surrogate" );
                    chars--;
                ...
                if(bytes + 1 >= byteEnd)
                {
                    ...
                    else
                    {
                        ...
                        BCLDebug.Assert( chars > charStart, "[UnicodeEncoding.GetBytes]Expected chars to have advanced for failed fallback" );
            ...
            BCLDebug.Assert( (encoder != null && !encoder.MustFlush) || charLeftOver == (char)0, "[UnicodeEncoding.GetBytes] Expected no left over characters if flushing" );

            BCLDebug.Assert( fallbackBuffer == null || fallbackBuffer.Remaining == 0 || encoder == null || !encoder.m_throwOnOverflow, "[UnicodeEncoding.GetBytes]Expected empty fallback buffer if not converting" );
        ...
        internal override unsafe int GetCharCount( byte*      bytes       ,
                                                   int        count       ,
                                                   DecoderNLS baseDecoder )
        {
            ...
            if(decoder != null)
            {
                ...
                // (don't have to check m_throwOnOverflow for count)
                BCLDebug.Assert( !decoder.InternalHasFallbackBuffer || decoder.FallbackBuffer.Remaining == 0, "[UnicodeEncoding.GetCharCount]Expected empty fallback buffer at start" );
            }
            ...
            // Shouldn't have anything in fallback buffer for GetCharCount
            // (don't have to check m_throwOnOverflow for count)
            BCLDebug.Assert( fallbackBuffer == null || fallbackBuffer.Remaining == 0, "[UnicodeEncoding.GetCharCount]Expected empty fallback buffer at end" );

            return charCount;
        }

        internal override unsafe int GetChars( byte*      bytes       ,
                                               int        byteCount   ,
                                               char*      chars       ,
                                               int        charCount   ,
                                               DecoderNLS baseDecoder )
        {
            ...
            if(decoder != null)
            {
                ...
                BCLDebug.Assert( !decoder.InternalHasFallbackBuffer || decoder.FallbackBuffer.Remaining == 0, "[UnicodeEncoding.GetChars]Expected empty fallback buffer at start" );
            }
            ...
            while(bytes < byteEnd)
            {
                ...
                if(ch >= 0xd800 && ch <= 0xdfff)
                {
                    ...
                    if(ch <= 0xdbff)
                    {
                        ...
                        if(lastChar > 0)
                        {
                            ...
                            if(!fallbackBuffer.InternalFallback( byteBuffer, bytes, ref chars ))
                            {
                                ...
                                BCLDebug.Assert( bytes >= byteStart + 2 || chars == charStart, "[UnicodeEncoding.GetChars]Expected bytes to have advanced or no output (bad surrogate)" );
                    ...
                    if(lastChar == 0)
                    {
                        ...
                        if(!fallbackBuffer.InternalFallback( byteBuffer, bytes, ref chars ))
                        {
                            // couldn't fall back lonely surrogate
                            // We either advanced bytes or chars should == charStart and throw below
                            BCLDebug.Assert( bytes >= byteStart + 2 || chars == charStart, "[UnicodeEncoding.GetChars]Expected bytes to have advanced or no output (lonely surrogate)" );
                            bytes -= 2;                                       // didn't use these 2 bytes
                            fallbackBuffer.InternalReset();
                            ThrowCharsOverflow( decoder, chars == charStart );// Might throw, if no chars output
                            break;                                          // couldn't fallback but didn't throw
                    ...
                    if(chars >= charEnd - 1)
                    {
                        ...
                        // We either advanced bytes or chars should == charStart and throw below
                        BCLDebug.Assert( bytes >= byteStart + 2 || chars == charStart, "[UnicodeEncoding.GetChars]Expected bytes to have advanced or no output (surrogate pair)" );
                        bytes -= 2;                                       // didn't use these 2 bytes
                        ThrowCharsOverflow( decoder, chars == charStart );// Might throw, if no chars output
                ...
                else if(lastChar > 0)
                {
                    ...
                    if(!fallbackBuffer.InternalFallback( byteBuffer, bytes, ref chars ))
                    {
                        ...
                        // We either advanced bytes or chars should == charStart and throw below
                        BCLDebug.Assert( bytes >= byteStart + 2 || chars == charStart, "[UnicodeEncoding.GetChars]Expected bytes to have advanced or no output (no low surrogate)" );
                        bytes -= 2;                                       // didn't use these 2 bytes
                        fallbackBuffer.InternalReset();
                        ThrowCharsOverflow( decoder, chars == charStart );// Might throw, if no chars output
                        break;                                          // couldn't fallback but didn't throw
                ...
                if(chars >= charEnd)
                {
                    ...
                    // We either advanced bytes or chars should == charStart and throw below
                    BCLDebug.Assert( bytes >= byteStart + 2 || chars == charStart, "[UnicodeEncoding.GetChars]Expected bytes to have advanced or no output (normal)" );
            ...
            if(decoder == null || decoder.MustFlush)
            {
                if(lastChar > 0)
                {
                    ...
                    if(!fallbackBuffer.InternalFallback( byteBuffer, bytes, ref chars ))
                    {
                        // 2 bytes couldn't fall back
                        // We either advanced bytes or chars should == charStart and throw below
                        BCLDebug.Assert( bytes >= byteStart + 2 || chars == charStart, "[UnicodeEncoding.GetChars]Expected bytes to have advanced or no output (decoder)" );
                        bytes -= 2;                                       // didn't use these bytes
                        if(lastByte >= 0)
                        {
            ...
            if(decoder != null)
            {
                BCLDebug.Assert( (decoder.MustFlush == false) || ((lastChar == (char)0) && (lastByte == -1)), "[UnicodeEncoding.GetChars] Expected no left over chars or bytes if flushing"
            ...
            BCLDebug.Assert( fallbackBuffer == null || fallbackBuffer.Remaining == 0, "[UnicodeEncoding.GetChars]Expected empty fallback buffer at end" );
