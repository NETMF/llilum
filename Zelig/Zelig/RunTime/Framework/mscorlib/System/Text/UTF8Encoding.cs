...
////    [System.Runtime.InteropServices.ComVisible( false )]
                                                   ...
                                                   EncoderNLS baseEncoder )
        {
            ...
            for(; ; )
            {
                ...
                if(pSrc >= pEnd)
                {
                    ...
                    else
                    {
                        ...
                        {
                            BCLDebug.Assert( ch >= 0xD800 && ch <= 0xDBFF, "[UTF8Encoding.GetBytes]expected high surrogate, not 0x" + ((int)ch).ToString( "X4", CultureInfo.InvariantCulture ) );
            ...
            BCLDebug.Assert( fallbackBuffer == null || fallbackBuffer.Remaining == 0, "[UTF8Encoding.GetByteCount]Expected Empty fallback buffer" );
        ...
        unsafe private static int PtrDiff( char* a ,
                                           char* b )
        {
        ...
        unsafe private static int PtrDiff( byte* a ,
                                           byte* b )
        {
            return (int)(a - b);
        }

        private static bool InRange( int ch    ,
                                     int start ,
                                     int end   )
        {
                                               ...
                                               EncoderNLS baseEncoder )
        {
            ...
            for(; ; )
            {
                ...
                if(pTarget > pAllocatedBufferEnd - bytesNeeded)
                {
                    ...
                    BCLDebug.Assert( pSrc >= chars || pTarget == bytes, "[UTF8Encoding.GetBytes]Expected pSrc to be within buffer or to throw with insufficient room." );
            ...
            if(encoder != null)
            {
                BCLDebug.Assert( !encoder.MustFlush || ch == 0, "[UTF8Encoding.GetBytes] Expected no mustflush or 0 leftover ch " + ch.ToString( "X2", CultureInfo.InvariantCulture ) );

                encoder.surrogateChar = ch;
                encoder.m_charsUsed = (int)(pSrc - chars);
            }

            BCLDebug.Assert( fallbackBuffer == null || fallbackBuffer.Remaining == 0 || baseEncoder == null || !baseEncoder.m_throwOnOverflow, "[UTF8Encoding.GetBytes]Expected empty fallback buffer if not converting" );
                                                   ...
                                                   DecoderNLS baseDecoder )
        {
            ...
            if(baseDecoder != null)
            {
                ...
                BCLDebug.Assert( !decoder.InternalHasFallbackBuffer || decoder.FallbackBuffer.Remaining == 0, "[UTF8Encoding.GetCharCount]Expected empty fallback buffer at start" );
            }
            ...
            for(; ; )
            {
                ...
                if((ch & FinalByte) == 0)
                {
                    BCLDebug.Assert( (ch & (SupplimentarySeq | ThreeByteSeq)) != 0, "[UTF8Encoding.GetChars]Invariant volation" );
            ...
            // Shouldn't have anything in fallback buffer for GetCharCount
            // (don't have to check m_throwOnOverflow for count)
            BCLDebug.Assert( fallback == null || fallback.Remaining == 0, "[UTF8Encoding.GetCharCount]Expected empty fallback buffer at end" );
                                               ...
                                               DecoderNLS baseDecoder )
        {
            ...
            if(baseDecoder != null)
            {
                ...
                BCLDebug.Assert( !decoder.InternalHasFallbackBuffer || decoder.FallbackBuffer.Remaining == 0, "[UTF8Encoding.GetChars]Expected empty fallback buffer at start" );
            }
            ...
            for(; ; )
            {
                ...
                if(!FallbackInvalidByteSequence( ref pSrc, ch, fallback, ref pTarget ))
                {
                    ...
                    BCLDebug.Assert( pSrc >= bytes || pTarget == chars, "[UTF8Encoding.GetChars]Expected to throw or remain in byte buffer after fallback" );
                    fallback.InternalReset();
                    ThrowCharsOverflow( baseDecoder, pTarget == chars );
                    ch = 0;
                    break;
                }
                BCLDebug.Assert( pSrc >= bytes, "[UTF8Encoding.GetChars]Expected invalid byte sequence to have remained within the byte array" );
                ch = 0;
                continue;
                ...
                if(pTarget >= pAllocatedBufferEnd)
                {
                    ...
                    BCLDebug.Assert( pSrc >= bytes || pTarget == chars, "[UTF8Encoding.GetChars]Expected pSrc to be within input buffer or throw due to no output]" );
                    ThrowCharsOverflow( baseDecoder, pTarget == chars );
            ...
            if(ch != 0 && (baseDecoder == null || baseDecoder.MustFlush))
            {
                ...
                if(!FallbackInvalidByteSequence( ref pSrc, ch, fallback, ref pTarget ))
                {
                    BCLDebug.Assert( pSrc >= bytes || pTarget == chars, "[UTF8Encoding.GetChars]Expected to throw or remain in byte buffer while flushing" );
                ...
                BCLDebug.Assert( pSrc >= bytes, "[UTF8Encoding.GetChars]Expected flushing invalid byte sequence to have remained within the byte array" );
                ch = 0;
            }
            ...
            if(baseDecoder != null)
            {
                ...
                BCLDebug.Assert( !baseDecoder.MustFlush || ch == 0 || !baseDecoder.m_throwOnOverflow, "[UTF8Encoding.GetChars]Expected no must flush or no left over bits or no throw on overflow." );
            ...
            BCLDebug.Assert( fallback == null || fallback.Remaining == 0, "[UTF8Encoding.GetChars]Expected empty fallback buffer at end" );
        ...
        private unsafe bool FallbackInvalidByteSequence( ref byte*                 pSrc     ,
                                                             int                   ch       ,
        ...
        private unsafe int FallbackInvalidByteSequence( byte*                 pSrc     ,
                                                        int                   ch       ,
                                                        DecoderFallbackBuffer fallback )
        {
            // Get our byte[]
