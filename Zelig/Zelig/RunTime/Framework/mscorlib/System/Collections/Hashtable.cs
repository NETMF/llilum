...
////[DebuggerDisplay( "Count = {Count}" )]
    ...
    public class Hashtable : IDictionary /*, ISerializable, IDeserializationCallback*/, ICloneable
    {
        ...
        */
        ...
        private volatile bool     m_isWriterInProgress;

        private ICollection       m_keys;
        private ICollection       m_values;

        private IEqualityComparer m_keycomparer;
...
////        get
////        {
////            if(m_keycomparer is CompatibleComparer)
////            {
////                return ((CompatibleComparer)m_keycomparer).HashCodeProvider;
////            }
////            else if(m_keycomparer == null)
////            {
////                return null;
////            }
////            else
...
////            if(m_keycomparer is CompatibleComparer)
////            {
////                CompatibleComparer keyComparer = (CompatibleComparer)m_keycomparer;
////
////                m_keycomparer = new CompatibleComparer( keyComparer.Comparer, value );
////            }
////            else if(m_keycomparer == null)
////            {
////                m_keycomparer = new CompatibleComparer( (IComparer)null, value );
////            }
////            else
////            {
////                throw new ArgumentException( Environment.GetResourceString( "Arg_CannotMixComparisonInfrastructure" ) );
...
////            if(m_keycomparer is CompatibleComparer)
////            {
////                return ((CompatibleComparer)m_keycomparer).Comparer;
////            }
////            else if(m_keycomparer == null)
////            {
////                return null;
...
////                CompatibleComparer keyComparer = (CompatibleComparer)m_keycomparer;
////
////                m_keycomparer = new CompatibleComparer( value, keyComparer.HashCodeProvider );
////            }
////            else if(m_keycomparer == null)
////            {
////                m_keycomparer = new CompatibleComparer( value, (IHashCodeProvider)null );
////            }
////            else
////            {
////                throw new ArgumentException( Environment.GetResourceString( "Arg_CannotMixComparisonInfrastructure" ) );
...
////    }
        ...
        public Hashtable( int capacity, float loadFactor )
        {
            ...
            BCLDebug.Assert( m_loadsize < hashsize, "Invalid hashtable loadsize!" );
        }
...
////        {
////            this.m_keycomparer = null;
////        }
////        else
////        {
////            this.m_keycomparer = new CompatibleComparer( comparer, hcp );
////        }
////    }

        public Hashtable( int capacity, float loadFactor, IEqualityComparer equalityComparer ) : this( capacity, loadFactor )
        {
            m_keycomparer = equalityComparer;
        }
...
////    [ReliabilityContract( Consistency.WillNotCorruptState, Cer.Success )]
        public virtual void Clear()
        {
            ...
            {
                m_buckets[i].hash_coll = 0;
                m_buckets[i].key       = null;
                m_buckets[i].val       = null;
            }
        ...
        {
            bucket[] lbuckets = m_buckets;

            Hashtable ht = new Hashtable( m_count, m_keycomparer );

            ht.m_version    = m_version;
            ht.m_loadFactor = m_loadFactor;
            ht.m_count      = 0;
        ...
        public virtual bool ContainsValue( Object value )
        {
            if(value == null)
            {
                ...
                {
                    if(m_buckets[i].key != null && m_buckets[i].key != m_buckets && m_buckets[i].val == null)
                    {
                        return true;
                    }
                }
        ...
        {
            KeyValuePairs[] array    = new KeyValuePairs[m_count];
            int             index    = 0;
            bucket[]        lbuckets = m_buckets;
        ...
        public virtual Object this[Object key]
        {
            get
            {
                ...
                do
                {
                    ...
                    } while(m_isWriterInProgress || (currentversion != m_version));
        ...
        private void expand()
        {
            int rawsize = HashHelpers.GetPrime( m_buckets.Length * 2 );    // buckets.Length*2 will not overflow
...
////    [ReliabilityContract( Consistency.WillNotCorruptState, Cer.MayFail )]
        private void rehash( int newsize )
        {
            ...
            for(int nb = 0; nb < m_buckets.Length; nb++)
            {
                bucket oldb = m_buckets[nb];

                if((oldb.key != null) && (oldb.key != m_buckets))
                {
            ...
            m_loadsize           = (int)(m_loadFactor * newsize);
            ...
            BCLDebug.Assert( m_loadsize < newsize, "Our current implementaion means this is not possible." );
            return;
        }
        ...
        {
            if(m_keycomparer != null)
            {
                return m_keycomparer.GetHashCode( key );
            }
        ...
        protected virtual bool KeyEquals( Object item, Object key )
        {
            ...
            if(Object.ReferenceEquals( m_buckets, item ))
            {
                return false;
            }
            ...
            if(m_keycomparer != null)
            {
                return m_keycomparer.Equals( item, key );
            }
        ...
        public virtual ICollection Keys
        {
            get
            {
                if(m_keys == null)
                {
                    m_keys = new Collection( this, true );
                }
        ...
        public virtual ICollection Values
        {
            get
            {
                if(m_values == null)
                {
                    m_values = new Collection( this, false );
                }
...
////    [ReliabilityContract( Consistency.WillNotCorruptState, Cer.MayFail )]
        private void Insert( Object key, Object nvalue, bool add )
        {
            ...
            if(m_count >= m_loadsize)
            {
                expand();
            }
            else if(m_occupancy > m_loadsize && m_count > 100)
            {
                rehash();
            }
            ...
            do
            {
                bucket b = lbuckets[bucketNumber];

                // Set emptySlot number to current bucket if it is the first available bucket that we have seen
                // that once contained an entry and also has had a collision.
                // We need to search this entire collision chain because we have to ensure that there are no
                // duplicate entries in the table.
                if(emptySlotNumber == -1 && b.key == lbuckets && b.hash_coll < 0)
                {
                ...
                if((b.key == null) || (b.key == lbuckets && b.hash_coll >= 0))
                {
                ...
                if(((b.hash_coll & 0x7FFFFFFF) == hashcode) && KeyEquals( b.key, key ))
                {
                    ...
                    {
#if EXCEPTION_STRINGS
                        throw new ArgumentException( Environment.GetResourceString( "Argument_AddingDuplicate__", b.key, key ) );
#else
                        throw new ArgumentException();
#endif
                    }
                ...
                if(emptySlotNumber == -1)
                {
                    ...
                    if(lbuckets[bucketNumber].hash_coll >= 0)
                    {
                        lbuckets[bucketNumber].hash_coll |= unchecked( (int)0x80000000 );
                        m_occupancy++;
                    }
                }
            ...
            if(emptySlotNumber != -1)
            {
                ...
                bucket b = m_buckets[emptySlotNumber];
        ...
        private void putEntry( bucket[] newBuckets, Object key, Object nvalue, int hashcode )
        {
            ...
            do
            {
                ...
                if((b.key == null) || (b.key == m_buckets))
                {
                    b.val        = nvalue;
                    b.key        = key;
                    b.hash_coll |= hashcode;
                    return;
                }

                if(b.hash_coll >= 0)
                {
                    b.hash_coll |= unchecked( (int)0x80000000 );
                    m_occupancy++;
                }
...
////    [ReliabilityContract( Consistency.WillNotCorruptState, Cer.MayFail )]
        public virtual void Remove( Object key )
        {
            ...
            uint hashcode = InitHash( key, m_buckets.Length, out seed, out incr );
            int  ntry     = 0;

            int    bucketNumber = (int)(seed % (uint)m_buckets.Length);  // bucketNumber
            bucket b;

            do
            {
                b = m_buckets[bucketNumber];
                ...
                if(((b.hash_coll & 0x7FFFFFFF) == hashcode) && KeyEquals( b.key, key ))
                {
                    ...
                    b.val = null;  // Free object references sooner & simplify ContainsValue.
                ...
                bucketNumber = (int)(((long)bucketNumber + incr) % (uint)m_buckets.Length);

            } while(b.hash_coll < 0 && ++ntry < m_buckets.Length);
...
////        }
////
////        info.AddValue( LoadFactorName, m_loadFactor );
...
////////    }
////////    else if(m_keycomparer is CompatibleComparer)
////////    {
////////        CompatibleComparer c = m_keycomparer as CompatibleComparer;
////////
...
////////    else
////////    {
////        info.AddValue( KeyComparerName, m_keycomparer, typeof( IEqualityComparer ) );
////////    }
////#pragma warning restore 618
////
////        info.AddValue( HashSizeName, m_buckets.Length ); //This is the length of the bucket array.
////
////        Object[] serKeys   = new Object[m_count];
////        Object[] serValues = new Object[m_count];
////
////        CopyKeys  ( serKeys  , 0 );
...
////            {
////                case LoadFactorName:
////                    m_loadFactor = m_siInfo.GetSingle( LoadFactorName );
////                    break;
////
////                case HashSizeName:
...
////                    m_keycomparer = (IEqualityComparer)m_siInfo.GetValue( KeyComparerName, typeof( IEqualityComparer ) );
////                    break;
////
////                case ComparerName:
...
////////    if((m_keycomparer == null) && ((c != null) || (hcp != null)))
////////    {
////////        m_keycomparer = new CompatibleComparer( c, hcp );
////////    }
////
////        m_buckets = new bucket[hashsize];
////
...
////    }
        ...
        // Implements a Collection for the keys or values of a hashtable. An instance of this
        // class is created by the GetKeys/GetValues method of a hashtable.
        [Serializable]
        private class Collection : ICollection
        {
            private Hashtable m_hashtable;
            private bool      m_keys;

            internal Collection( Hashtable hashtable, bool keys )
            {
            ...
            public virtual void CopyTo( Array array, int arrayIndex )
            {
                ...
                if(array.Length - arrayIndex < m_hashtable.m_count)
                {
#if EXCEPTION_STRINGS
                    throw new ArgumentException( Environment.GetResourceString( "Arg_ArrayPlusOffTooSmall" ) );
#else
                ...
                if(m_keys)
                {
                    m_hashtable.CopyKeys( array, arrayIndex );
                }
                else
                {
                    m_hashtable.CopyValues( array, arrayIndex );
                }
            }

            public virtual IEnumerator GetEnumerator()
            {
                return new HashtableEnumerator( m_hashtable, m_keys ? HashtableEnumerator.Keys : HashtableEnumerator.Values );
            }
...
////        {
////            m_table = (Hashtable)info.GetValue( "ParentTable", typeof( Hashtable ) );
////            if(m_table == null)
////            {
...
////            }
////
////            info.AddValue( "ParentTable", m_table, typeof( Hashtable ) );
////        }
////
...
////            lock(m_table.SyncRoot)
////            {
////                return m_table.ContainsValue( key );
////            }
////        }
////
...
////            lock(m_table.SyncRoot)
////            {
////                return Hashtable.Synchronized( (Hashtable)m_table.Clone() );
////            }
////        }
////
////        IEnumerator IEnumerable.GetEnumerator()
...
////        {
////            return m_table.ToKeyValuePairsArray();
////        }
////    }
        ...
        private class HashtableEnumerator : IDictionaryEnumerator, ICloneable
        {
            ...
            {
                m_hashtable        = hashtable;
                m_bucket           = hashtable.m_buckets.Length;
                m_version          = hashtable.m_version;
                m_current          = false;
            ...
            {
                if(m_version != m_hashtable.m_version)
                {
#if EXCEPTION_STRINGS
                ...
                while(m_bucket > 0)
                {
                    ...
                    bucket[] lbuckets = m_hashtable.m_buckets;

                    Object keyv = lbuckets[m_bucket].key;

                    if((keyv != null) && (keyv != lbuckets))
                    {
                        m_currentKey   = keyv;
                        m_currentValue = lbuckets[m_bucket].val;
                        m_current      = true;
                        return true;
                    }
                }
            ...
            public virtual DictionaryEntry Entry
            {
                get
                {
                    ...
                    return new DictionaryEntry( m_currentKey, m_currentValue );
                }
            }
            ...
            public virtual void Reset()
            {
                ...
                m_current      = false;
                m_bucket       = m_hashtable.m_buckets.Length;
                m_currentKey   = null;
                m_currentValue = null;
            }
        }
