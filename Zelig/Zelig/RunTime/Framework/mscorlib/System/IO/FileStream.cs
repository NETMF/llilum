...
namespace System.IO
{
    ...
    public class FileStream : Stream
    {
        ...
        public FileStream( String path, FileMode mode, FileAccess access, FileShare share, int bufferSize )
        {
            ...
            if(mode != FileMode.Open && mode != FileMode.OpenOrCreate) // && !wantsWrite) // TODO: BUGBUG: this line produces a compiler bug
            {
                // this line added because of bug above
            ...
            try
            {
                ...
                switch(mode)
                {
                    ...
                    case FileMode.Open: // if the file does not exist, IOException/FileNotFound is thrown
                        if(!exists) 
#if EXCEPTION_STRINGS
                            throw new IOException( "", (int)IOExceptionErrorCode.FileNotFound );
#else
                            throw new IOException();
#endif
                        _nativeFileStream = new NativeFileStream( _fileName, bufferSize );
                ...
                // ZeligBUG - this fixes bug above... (also, for some reason, wantsWrite is not being compiled correctly)
                if(( access & FileAccess.Write ) == FileAccess.Write && isReadOnly)
                {
#if EXCEPTION_STRINGS
                    throw new IOException( "", (int)IOExceptionErrorCode.UnauthorizedAccess );
#else
        ...
        public override int Read( byte[] buffer, int offset, int count )
        {
            if(_disposed) 
#if EXCEPTION_STRINGS
                throw new ObjectDisposedException( typeof( FileStream ).ToString() );
#else
                throw new ObjectDisposedException( null );
#endif
