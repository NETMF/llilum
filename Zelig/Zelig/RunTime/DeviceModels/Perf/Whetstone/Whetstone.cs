...
namespace WhetstoneTest
{
    ...
    */
    public class WhetstoneTest
    {
        double T, T1, T2;
        double[] E1 = new double[5];
        int J, K, L, iKIPS;

        public static void Main()
        {
            WhetstoneTest tst = new WhetstoneTest();

            tst.TestMain( 500 );
        }
        public void TestMain( int iterations )
        {
...
*/
            ...
            */
            ...
            {
#if PRINTOUT
                Console.WriteLine( "Insufficient duration- Increase the LOOP count" );
#endif
                return;
            }

#if PRINTOUT
            Console.WriteLine( "Loops: {0}, Iterations: {1}, Duration: {2} sec.",
                    LOOP, II, finisec - startsec );
#endif
            KIPS = ( 100.0 * LOOP * II ) / (double)( finisec - startsec ).TotalSeconds;

            iKIPS = (int)KIPS;
#if PRINTOUT
            if(KIPS >= 1000.0)
                Console.WriteLine( "C Converted Double Precision Whetstones: {0} MIPS", KIPS / 1000.0 );
            else
                Console.WriteLine( "C Converted Double Precision Whetstones: {0} KIPS", KIPS );
#endif
        ...
        void
        P3( double X, double Y, ref double Z )
        {
            double X1, Y1;
...
{
	Console.WriteLine("%7ld %7ld %7ld %12.4e %12.4e %12.4e %12.4e",
						N, J, K, X1, X2, X3, X4);
}
#endif
    }
