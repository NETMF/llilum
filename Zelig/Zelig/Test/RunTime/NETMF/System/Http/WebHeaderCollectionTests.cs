...
{
    public class WebHeaderCollectionTests : TestBase, ITestInterface
    {
        [SetUp]
        ...
        private TestResult VerifyHttpHeaders(System.Net.WebHeaderCollection wrc, System.Net.WebHeaderCollection wrs)
        {
        ...
        #region Test
        [TestMethod]
        public TestResult TestWebHeaderCollectionAddIllegal()
        {
            TestResult result = TestResult.Pass;
            ...
            try
            {
                ...
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Accept, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Connection, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.ContentLength, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.ContentType, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Date, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Expect, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Host, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.IfModifiedSince, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Range, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Referer, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.TransferEncoding, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.UserAgent, null, typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.ProxyConnection, null, typeof(ArgumentException))) result = TestResult.Fail;
        ...
        [TestMethod]
        public TestResult TestWebHeaderCollectionAddIllegal2()
        {
            TestResult result = TestResult.Pass;
            ...
            try
            {
                ...
                //Attempt to add Restricted header
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Accept, "text/plain; q=0.5, text/html, text/x-dvi; q=0.8, text/x-c", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Connection, "close", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.ContentLength, "26012", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.ContentType, "image/gif", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Date, System.DateTime.Today.ToString(), typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Expect, "100", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Host, "www.w3.org", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.IfModifiedSince, "Sat, 23 May 2009 19:43:31 GMT", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Range, "500-999", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.Referer, "http://www.w3.org/hypertext/DataSources/Overview.html", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.TransferEncoding, "chunked", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.UserAgent, ".NetMicroFramework", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyAddHeaderIsIllegal(wrc, HttpKnownHeaderNames.ProxyConnection, "www.microsoft.com", typeof(ArgumentException))) result = TestResult.Fail;
        ...
        [TestMethod]
        public TestResult TestWebHeaderCollectionAddLegal2()
        {
            TestResult result = TestResult.Pass;
        ...
        [TestMethod]
        public TestResult TestWebHeaderCollectionIsRestricted()
        {
            TestResult result = TestResult.Pass;
            ...
            try
            {
                ...
                //Attempt to add Restricted header
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Accept)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Connection)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.ContentLength)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.ContentType)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Date)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Expect)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Host)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.IfModifiedSince)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Range)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.Referer)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.TransferEncoding)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.UserAgent)) result = TestResult.Fail;
                if (!WebHeaderCollection.IsRestricted(HttpKnownHeaderNames.ProxyConnection)) result = TestResult.Fail;
        ...
        [TestMethod]
        public TestResult TestWebHeaderCollectionRemove()
        {
            TestResult result = TestResult.Pass;
            HttpWebRequest wr = (HttpWebRequest)WebRequest.Create("http://127.0.0.1:" + HttpServer.s_CurrentPort.ToString() + "/");
            wr.UserAgent = ".Net Micro Framwork Device/4.0";
