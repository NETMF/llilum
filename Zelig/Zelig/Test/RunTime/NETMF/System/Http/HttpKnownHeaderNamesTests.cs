...
{
    public class HttpKnownHeaderNamesTests : TestBase, ITestInterface
    {
        [SetUp]
        ...
        [TestMethod]
        public TestResult TestSetHTTPRequestHeaderAfterCreateHTTP1_1()
        {
            ...
            try
            {
                ...
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Accept, "text/plain; q=0.5, text/html, text/x-dvi; q=0.8, text/x-c", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Connection, "close", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.ContentLength, "26012", typeof(ArgumentException))) result = TestResult.Fail;
                //if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.ContentType, "image/gif", typeof(ArgumentException))) result = TestResult.Fail;
                //if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Date, System.DateTime.Today.ToString(), typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Expect, "100", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Host, "www.w3.org", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.IfModifiedSince, "Sat, 23 May 2009 19:43:31 GMT", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Range, "500-999", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.Referer, "http://www.w3.org/hypertext/DataSources/Overview.html", typeof(ArgumentException))) result = TestResult.Fail;                
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.TransferEncoding, "chunked", typeof(ArgumentException))) result = TestResult.Fail;
                if (!VerifyHeaderIsIllegal(wrc, HttpKnownHeaderNames.UserAgent, ".NetMicroFramework", typeof(ArgumentException))) result = TestResult.Fail;
        ...
        public TestResult ValidateAbleToSetPropertiesValueHTTP1_1()
        {
            TestResult result = TestResult.Pass;
            HttpWebRequest wr = (HttpWebRequest)WebRequest.Create("http://127.0.0.1:" + HttpServer.s_CurrentPort.ToString() + "/");
            wr.UserAgent = ".Net Micro Framwork Device/4.0";
        ...
        [TestMethod]
        public TestResult TestSetHTTPRequestHeaderAfterCreateHTTP1_0()
        {
            TestResult result = TestResult.Pass;
            HttpWebRequest wr = (HttpWebRequest)WebRequest.Create("http://127.0.0.1:" + HttpServer.s_CurrentPort.ToString() + "/");
            wr.UserAgent = ".Net Micro Framwork Device/4.0";
        ...
        public TestResult ValidateAbleToSetPropertiesValueHTTP1_0()
        {
            TestResult result = TestResult.Pass;
            HttpWebRequest wr = (HttpWebRequest)WebRequest.Create("http://127.0.0.1:" + HttpServer.s_CurrentPort.ToString() + "/");
            wr.UserAgent = ".Net Micro Framwork Device/4.0";
